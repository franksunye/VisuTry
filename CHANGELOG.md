...
- Stripe切换为正式环境
- Premium用户年费配额显示修复（需要数据库迁移）

## [0.3.0] - 2025-11-05

### ✨ 新功能
- 主页展示轮播组件：添加交互式Try-On展示轮播
- 多语言支持：为主页展示轮播添加9种语言支持（英、西、法、德、日、印尼、葡、俄、阿）

### 🐛 Bug修复
- 修复配额计算逻辑：正确计算Free Trials + Credits的总数
- 修复Premium用户配额计算：使用正确的使用计数而非Free Trials计数
- 修复TryOnShowcase组件TypeScript错误：正确使用i18n属性引用
- 修复UserStatusBanner特殊字符转义：正确处理撇号
- 移除非SEO页面的翻译调用：优化i18n实现

### ♻️ 重构
- 登录页面优化：移除重复的logo和title元素，改进布局
- Hero区域优化：优化间距和排版

### 💄 样式
- 调整Hero标题字体大小：从4xl/5xl降低到3xl/4xl
- 紧凑配额显示：将配额显示改为单行
