...
The purge client now routes the bounded `public_html_invalidation`
success/failure event through the canonical `src/lib/logger.ts` → Axiom path,
using the existing `event`, `type`, `status`, `source`, and bounded
`failureReason` fields. Resource counts remain in the unchanged
`PublicHtmlPurgeResult`; no new Axiom schema fields were introduced. Purge,
retry, write, and fail-open semantics remain unchanged, and logger/Axiom
failure remains non-blocking. This code-level gap is closed pending ordinary
deployment observation; no new production Campaign write proof is claimed or
required.
