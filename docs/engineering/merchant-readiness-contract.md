...
- **Recommendation eligible**: the frame is active, catalog eligible, has the required eyewear attributes (currently shape), and its `enrichmentStatus` is `APPROVED` or `NOT_REQUIRED`.
...
- `NOT_REQUIRED`: the frame is explicitly exempt from enrichment requirements.

Missing SKU never blocks a stable URL/external-id import. Missing shape never blocks catalog import; it only prevents recommendation readiness and is persisted as `PENDING` unless a lower-confidence inference is explicitly marked `REVIEW_REQUIRED`.
