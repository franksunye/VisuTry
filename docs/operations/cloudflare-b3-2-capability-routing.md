...
Recorded B3.2 staging validation: `npm ci`, typecheck, lint (existing warnings only), critical tests (7 suites / 30 tests), router tests (5 / 5), `build:ci`, `build:cloudflare:next`, OpenNext Cloudflare bundle build, and Wrangler staging dry-run. The final staging matrix is recorded above. Reconciliation validation is in **Main Reconciliation**. No production cutover, production DNS change, production route, or production deployment is authorized by this document.

## Main Reconciliation

This section records bringing the already-proven B3.2 implementation onto current `main` without redesigning capability routing or moving production traffic.

| Item | Value |
| --- | --- |
| `origin/main` | `5147c3c02996771e07d084c9549adb9876640a0a` (includes PR #91) |
| Cloudflare source branch | `origin/codex/cloudflare-phase-a-build-parity` @ `615de7a0d3824082764a5c91273e50c6cb037b0d` |
| B3.2 finalize commit | `4a496700825e1085c97ba4f5bc013c2c7c594770` |
| Reconciliation branch | `cursor/cloudflare-b3-2-main-reconciliation` |

PR #91 quota behavior is preserved on the Vercel production path:

- brand/category/try remain `force-static` with no hourly ISR
- Store/Campaign keep the 7-day safety TTL plus existing on-demand invalidation
- public glasses/catalog GET CDN caching is unchanged
- locale-less redirects and the narrowed middleware matcher remain
- `/api/frames` public DTO contract remains

Cloudflare OpenNext still uses `CLOUDFLARE_BUILD=1` webpack aliases, the Prisma stub, and direct-Neon providers. `dynamicParams` opens only during that Cloudflare build so OpenNext can dispatch nested pages; the Vercel default stays closed.

| Measurement | Value |
| --- | ---: |
| B3.2 proven Worker | 2,781.46 KiB gzip |
| Reconciled dry-run Worker | 2,777.11 KiB gzip |
| Delta vs proven B3.2 | -4.35 KiB |
| Free-plan limit | 3,072 KiB |
| Headroom | 294.89 KiB |
| Prisma query compiler / WASM | absent |
