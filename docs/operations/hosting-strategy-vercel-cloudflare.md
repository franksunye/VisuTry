...
The current production data plane is still the validated 19-route / seven-URL Consumer slice. The reviewed Store/Campaign v1 implementation on the feature branch proposes two additional bounded EN Worker invocation routes (`/en/store/*` and `/en/c/*`). These routes do not create a second Next producer: Vercel continues to produce canonical HTML, while the Worker may cache only safe final anonymous HTML and proxies every unsafe or unknown variant to Vercel. Store/Campaign freshness is write-driven through exact post-commit URL invalidation, not the seven-URL release warm-up list. The edge query contract allows only attribution and bounded client continuation keys whose values do not affect server HTML; unknown keys, RSC/Flight, cookies, authorization, preview, and personalization bypass to Vercel.
