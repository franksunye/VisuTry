...
describe('MerchantControlCenter', () => {
  ...
  it('shows a newly created secret once and removes it on close', async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringContaining('- get_merchant\n- get_onboarding_status'))
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringContaining('Do not stop after reporting the connection status.'))
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringContaining('If no Store exists:'))
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringContaining('What would you like this Campaign to achieve?'))
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringContaining('recommend the next useful action.'))
