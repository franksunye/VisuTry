...
describe('HTTP MCP OAuth discovery and connection chain', () => {
  beforeEach(() => {
    ...
    mockExchange.mockResolvedValue({ accessToken: 'mcp_at_a', refreshToken: 'mcp_rt_a', expiresIn: 3600, scope: 'merchant:read', resource: 'http://localhost:3000/api/mcp' })
    mockAuthenticate.mockRejectedValue(new (require('@/modules/merchant').InvalidAgentCredentialError)())
  })
  ...
  it('completes discovery, registration, PKCE authorization, token exchange, and MCP initialize/tools list over HTTP handlers', async () => {
    const unauthenticated = await mcpPost(request('http://localhost:3000/api/mcp', { method: 'POST', headers: { authorization: 'Bearer missing', 'content-type': 'application/json' }, body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'initialize', params: {} }) }))
    ...
    const authorization = await authorizePost(request('http://localhost:3000/api/mcp/oauth/authorize', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ request_id: 'request-a', merchant_id: 'merchant-a', decision: 'allow' }).toString() }))
    ...
    const token = await tokenPost(request('http://localhost:3000/api/mcp/oauth/token', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ grant_type: 'authorization_code', client_id: 'mcp_client_a', code: 'code-a', redirect_uri: 'http://127.0.0.1:4567/callback', code_verifier: 'verifier-a', resource: 'http://localhost:3000/api/mcp' }).toString() }))
    expect(token.status).toBe(200)
    expect(await token.json()).toMatchObject({ access_token: 'mcp_at_a', refresh_token: 'mcp_rt_a', resource: 'http://localhost:3000/api/mcp' })
    expect(mockExchange).toHaveBeenCalledWith(expect.objectContaining({ resource: 'http://localhost:3000/api/mcp', codeVerifier: 'verifier-a' }))

    mockAuthenticate.mockResolvedValue({ actorType: 'AGENT_CREDENTIAL', actorId: 'credential-a', merchantId: 'merchant-a', scopes: ['merchant:read', 'catalog:read', 'experience:read', 'analytics:read'] })
    const initialize = await mcpPost(request('http://localhost:3000/api/mcp', { method: 'POST', headers: { authorization: 'Bearer mcp_at_a', 'content-type': 'application/json', accept: 'application/json, text/event-stream' }, body: JSON.stringify({ jsonrpc: '2.0', id: 2, method: 'initialize', params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'integration-test', version: '1' } } }) }))
    expect(initialize.status).toBe(200)
    expect((await initialize.json() as any).result.instructions).toContain('one authorized VisuTry Merchant workspace')

    const list = await mcpPost(request('http://localhost:3000/api/mcp', { method: 'POST', headers: { authorization: 'Bearer mcp_at_a', 'content-type': 'application/json', accept: 'application/json, text/event-stream' }, body: JSON.stringify({ jsonrpc: '2.0', id: 3, method: 'tools/list', params: {} }) }))
    expect(list.status).toBe(200)
    expect((await list.json() as any).result.tools).toEqual(expect.arrayContaining([expect.objectContaining({ name: 'get_merchant' }), expect.objectContaining({ name: 'create_campaign' })]))
  })
})
