...
describe('Merchant Skill endpoint', () => {
  it('is public, cacheable, and combines setup, campaign, and analytics guidance', async () => {
    ...
    expect(text).toMatch(/inspect_catalog_source/iu)
    expect(text).toMatch(/returns a proposal/iu)
    expect(text).toMatch(/never let a website URL automatically create a Store/iu)
    expect(text).toMatch(/publish_campaign requires explicit approval/iu)
    expect(text).not.toMatch(/create_website_crawler|scrape_catalog|invented_tool|generic browser automation/iu)
