...
describe('Merchant Agent integration status presentation', () => {
  it('distinguishes no active credential from revoked history', () => {
    expect(resolveMerchantAgentKeyAccessStatus([])).toMatchObject({ kind: 'NOT_CONFIGURED', activeCredentialCount: 0, revokedCredentialCount: 0 })
    expect(resolveMerchantAgentKeyAccessStatus([{ status: 'REVOKED', lastUsedAt: '2026-09-01T00:00:00.000Z' }])).toMatchObject({ kind: 'NOT_CONFIGURED', activeCredentialCount: 0, revokedCredentialCount: 1, latestActiveUseAt: null })
  })

  it('does not treat an active unused credential as connected or used', () => {
    expect(resolveMerchantAgentKeyAccessStatus([{ status: 'ACTIVE', lastUsedAt: null }])).toEqual({ kind: 'READY_TO_CONNECT', activeCredentialCount: 1, revokedCredentialCount: 0, latestActiveUseAt: null })
  })

  it('uses real lastUsedAt on an active key as evidence and chooses the latest timestamp', () => {
    expect(resolveMerchantAgentKeyAccessStatus([
  ...
  it('does not accept malformed usage timestamps as connection evidence', () => {
    expect(resolveMerchantAgentKeyAccessStatus([{ status: 'ACTIVE', lastUsedAt: 'not-a-date' }]).kind).toBe('READY_TO_CONNECT')
  })
})
