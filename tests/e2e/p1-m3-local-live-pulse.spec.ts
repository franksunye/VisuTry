...
test.describe('P1-M3 Local Live Commerce Pulse', () => {
  test('shows a truthful quiet state, then real Store activity on Home without reload', async ({ page }) => {
    ...
    console.log(JSON.stringify({ startedEmpty, desktopQuietMetrics, desktopLiveMetrics, mobileLiveMetrics, activityUpdatedWithoutReload: pulseRequestsAfter > pulseRequestsBefore }))
    for (const metrics of [desktopQuietMetrics, desktopLiveMetrics, mobileLiveMetrics]) expect(metrics.horizontalOverflow).toBe(false)
