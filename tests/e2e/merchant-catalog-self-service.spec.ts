...
test.describe('Human Merchant Catalog G2', () => {
  test('covers URL, CSV, and Manual preview-to-approval paths', async ({ page, request, context }) => {
    ...
    await page.getByLabel(/product name for product 1/i).fill('Manual Frame')
    await page.getByLabel(/product image url for product 1/i).fill('https://cdn.example.test/manual.jpg')
    await page.getByLabel(/merchant sku for product 1/i).fill('G2-3')
