...
test.describe('P1-M2.5 Local Campaign workspace', () => {
  test('runs real Local draft, preview, entitlement, publish, live edit and archive journeys', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/en\/merchant\/campaigns(?:\?|$)/)
    await expect(page.getByTestId('merchant-campaign-workspace')).toBeVisible()
    await expect(page.getByText('No Campaigns yet')).toBeVisible()
    await capture('01-campaign-list-empty-desktop')
    await page.setViewportSize({ width: 390, height: 844 })
    await capture('09-campaign-list-mobile')
    await page.setViewportSize({ width: 1440, height: 900 })
    ...
    await page.getByLabel('Shopper-facing headline').fill('A refreshed story while product selection is saved')
    journeyClicks.createToPrivatePreview += 1
    const productsPut = page.waitForRequest((request) => new URL(request.url()).pathname.endsWith('/products') && request.method() === 'PUT')
    await page.getByRole('button', { name: 'Save product selection' }).click()
    await productsPut
    journeyClicks.createToPrivatePreview += 1
    await expect(page.getByText('Product selection saved.')).toBeVisible()
    await expect(page.getByText('Unsaved detail changes')).toBeVisible()
    await expect(page.getByRole('button', { name: 'Save campaign details' })).toBeEnabled()
    await expect(page.getByRole('button', { name: 'Private Preview' })).toBeDisabled()
    const detailsPatch = page.waitForRequest((request) => request.url().includes('/campaigns/') && request.method() === 'PATCH')
    ...
    await page.getByLabel('Shopper-facing headline').fill('Unsaved headline must not be lost')
    await expect(page.getByRole('button', { name: 'Archive Campaign' })).toBeDisabled()
    await expect(page.getByText('Save your changes before archiving.')).toBeVisible()
    await page.getByLabel('Shopper-facing headline').fill('Updated for shoppers immediately')
    await expect(page.getByRole('button', { name: 'Archive Campaign' })).toBeEnabled()
