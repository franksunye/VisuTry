...
export function MerchantPurchaseSummary({ locale, merchantId, merchantName, intent, plan, action, currentPlanName }: Props) {
  ...
  const isBlocked = action === 'CURRENT' || action === 'DUPLICATE_PILOT'
      ...
      <section className="mx-auto max-w-3xl rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_25px_80px_-55px_rgba(15,23,42,0.55)] sm:p-10">
        ...
        <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            {currentPlanName && action !== 'CURRENT' ? <p className="mt-2 text-sm text-slate-600">Current plan: {currentPlanName}</p> : null}
          </div>
        ...
        {action === 'DUPLICATE_PILOT' ? <p className="mt-4 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm leading-6 text-blue-900" role="status">Your Founding Pilot is already active. Your Store and catalog remain available while you continue with the current Pilot.</p> : null}
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
          {isBlocked ? <a href={`/${locale}/merchant?merchantId=${encodeURIComponent(merchantId)}`} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 py-3.5 text-sm font-semibold text-white hover:bg-slate-800">Go to Merchant workspace <ArrowRight className="h-4 w-4" aria-hidden="true" /></a> : <button type="button" disabled={busy} onClick={() => void continuePurchase()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 py-3.5 text-sm font-semibold text-white hover:bg-slate-800 disabled:cursor-wait disabled:opacity-60">{busy ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> : null}{actionCopy(action, plan)}{!busy ? <ArrowRight className="h-4 w-4" aria-hidden="true" /> : null}</button>}
