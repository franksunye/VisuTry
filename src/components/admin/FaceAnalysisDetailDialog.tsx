...
import FaceAnalysisAdminReport from '@/components/admin/FaceAnalysisAdminReport';
...
export default function FaceAnalysisDetailDialog({
  ...
  const hasReport = task.status === 'COMPLETED' && (task.fullResult || task.basicResult);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {hasReport ? (
            <FaceAnalysisAdminReport
              userImageUrl={task.userImageUrl}
              detectedShape={task.detectedShape}
              confidence={task.confidence}
              basicResult={task.basicResult}
              fullResult={task.fullResult}
              metadata={task.metadata}
            />
          ) : (
            <p className="text-sm text-muted-foreground">No analysis report available for this task.</p>
          )}
        </div>
