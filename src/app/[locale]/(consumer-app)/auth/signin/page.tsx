...
export default async function SignInPage(props: Props) {
  ...
  return (
    <main data-auth-surface={isShopperContinuation ? 'shopper' : isMerchantAcquisition ? 'merchant-admin' : 'consumer'} className="min-h-screen bg-[#f7f8fb] px-4 py-10 text-slate-950 sm:px-6 sm:py-16 lg:px-8">
      ...
      {isMerchantAcquisition ? <p className="mx-auto mt-5 max-w-5xl text-center text-xs text-slate-500">This is the merchant workspace. For virtual try-on, use the <a href={localizedPath(params.locale, '/try-on')} className="text-blue-700 hover:underline">shopper experience</a>.</p> : null}
    </main>
  )
}
