    ...
    <main className="min-h-screen bg-[#f7f8fb] px-4 py-10 text-slate-950 sm:px-6 sm:py-16 lg:px-8">
      <div className="mx-auto grid max-w-5xl overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_30px_90px_-55px_rgba(15,23,42,0.55)] lg:grid-cols-[1.05fr_0.95fr]">
        <section className="bg-[radial-gradient(circle_at_80%_0%,rgba(191,219,254,0.5),transparent_40%),linear-gradient(145deg,#f8fbff,#eef6ff)] p-7 sm:p-10 lg:p-12">
          ...
          <p className="mt-8 text-xs font-bold uppercase tracking-[0.18em] text-blue-700">VisuTry for merchants</p>
          <h1 className="mt-3 max-w-lg text-3xl font-semibold tracking-[-0.04em] text-slate-950 sm:text-5xl">Connect your store to the AI agent you already use.</h1>
          <p className="mt-5 max-w-lg text-base leading-7 text-slate-600">Create a merchant workspace, copy the Key + Skill setup, and paste it into ChatGPT, Claude, or another agent. Your agent will guide the rest.</p>
        ...
        <section className="p-7 sm:p-10 lg:p-12">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-slate-400"><ShieldCheck className="h-4 w-4 text-emerald-600" aria-hidden="true" /> Secure merchant access</div>
          <h2 className="mt-5 text-2xl font-semibold tracking-tight text-slate-950">Start your merchant setup</h2>
          <p className="mt-3 text-sm leading-6 text-slate-600">Use a work email for your brand. New accounts go straight to Merchant setup; existing VisuTry accounts can sign in and choose a workspace.</p>
          <div className="mt-8"><MerchantAuthActions callbackUrl={merchantCallback} /></div>
          <div className="mt-8 rounded-2xl border border-blue-100 bg-blue-50/60 p-4"><div className="flex items-start gap-3"><KeyRound className="mt-0.5 h-5 w-5 shrink-0 text-blue-700" aria-hidden="true" /><p className="text-sm leading-6 text-slate-700"><span className="font-semibold text-slate-900">What happens next?</span><br />Create your key; VisuTry copies the Key + Skill setup, ready to paste into your agent chat.</p></div></div>
      ...
      <p className="mx-auto mt-5 max-w-5xl text-center text-xs text-slate-500">This is the merchant workspace. For virtual try-on, use the <a href={localizedPath(params.locale, '/try-on')} className="text-blue-700 hover:underline">shopper experience</a>.</p>
    </main>
  )
}
