...
import { getGlassesGuideHubCopy } from '@/config/search-to-tool-locales'
import type { Locale } from '@/i18n'
import { generateI18nSEO, generateStructuredData } from '@/lib/seo'

const pathname = '/glasses-guide'
...
export default function GlassesGuideHub({ params }: Props) {
  ...
  const collectionSchema = generateStructuredData('breadcrumbList', {
    items: [
      { name: copy.homeBreadcrumb, url: `/${locale}` },
      { name: copy.guidesBreadcrumb, url: `/${locale}${pathname}` },
    ],
  })
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 md:py-14">
        ...
        {locale !== 'en' && copy.phaseBNote && (
          <div className="mt-8 max-w-3xl rounded-lg border border-blue-100 bg-blue-50/60 px-5 py-4 text-sm leading-6 text-gray-700">
            {copy.phaseBNote}
          </div>
        )}
