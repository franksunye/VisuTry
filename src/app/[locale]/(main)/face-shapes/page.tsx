      ...
      <main className="mx-auto max-w-6xl px-4 py-8 md:py-12">
        ...
        <section className="mt-12">
          <div className="mb-5 max-w-3xl">
            <h2 className="mb-3 text-2xl font-bold text-gray-950">Compare commonly confused face shapes</h2>
            <p className="leading-7 text-gray-600">
              When two categories seem close, compare the widest point, side outline, and jaw
              structure instead of relying on one length ratio.
            </p>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {FACE_SHAPE_COMPARISON_SLUGS.map((slug) => {
              const comparison = FACE_SHAPE_COMPARISONS[slug]
              const first = FACE_SHAPE_CONTENT[comparison.first]
              const second = FACE_SHAPE_CONTENT[comparison.second]
              return (
                <Link
                  key={slug}
                  href={`/${locale}/face-shapes/compare/${slug}`}
                  className="group rounded-lg border border-gray-200 bg-white p-5 hover:border-blue-300 hover:shadow-sm"
                >
                  <h3 className="mb-2 text-lg font-semibold text-gray-950 group-hover:text-blue-700">
                    {first.name} vs {second.name}
                  </h3>
                  <p className="text-sm leading-6 text-gray-600">{comparison.keyDifference}</p>
                </Link>
              )
            })}
          </div>
          <Link href={`/${locale}/face-shape-measurement`} className="mt-5 inline-flex items-center gap-1 font-semibold text-blue-700 hover:text-blue-900">
            Measure your proportions manually <ArrowRight className="h-4 w-4" />
          </Link>
        </section>
