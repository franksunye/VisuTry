...
export async function POST(request: NextRequest, { params }: { params: { merchantId: string } }) {
  ...
  try {
    ...
    if (!parsed.success) return NextResponse.json({ success: false, error: 'APPROVAL_REQUIRED', message: 'Explicit approval is required before publishing.' }, { status: 400 })
    const data = await publishMerchantStore({ actor: { actorType: 'HUMAN', actorId: auth.userId, merchantId: params.merchantId, membershipId: membership.membershipId }, ...parsed.data })
