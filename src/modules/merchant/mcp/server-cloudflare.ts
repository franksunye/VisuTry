...
export function createMerchantMcpServer(actor: AgentMerchantActor) {
  ...
  server.registerTool('import_frames', { title: 'Import frames', description: 'Idempotently import up to 1,000 structured catalog frames.', inputSchema: { frames: z.array(frameInput).min(1).max(1000) } }, async ({ frames }) => safe(() => merchantOnboarding.importMerchantFrames({ actor, frames: frames as CatalogFrameInput[] })))
  ...
  server.registerTool('create_store', { title: 'Create Store', description: 'Create or reuse a Store DRAFT.', inputSchema: { name: z.string().max(120).optional(), headline: z.string().max(240).optional(), description: z.string().max(5000).optional() } }, async ({ name, headline, description }) => safe(() => merchantOnboarding.createMerchantStore({ actor, name, headline, description })))
  server.registerTool('set_store_frames', { title: 'Set Store frames', description: 'Replace the Store frame selection within the authenticated tenant.', inputSchema: { storeId: z.string().min(1), frameIds: z.array(z.string().min(1)).max(MAX_CATALOG_IMPORT) } }, async ({ storeId, frameIds }) => safe(() => merchantOnboarding.setMerchantStoreFrames({ actor, storeId, frameIds })))
